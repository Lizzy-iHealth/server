...
public class InitServlet extends APIServlet {
  ...
  public void handle(HttpServletRequest req, HttpServletResponse resp)
    ...
    initBank();
    User taskAdmin = initTaskAdmin();
    createDailyQuest("Check In", "When you check in, your friends will see your location.",dailyQuestURL, taskAdmin); 
  }

protected void createDailyQuest(String title, String description, String dailyQuestURL, User taskAdmin) {
	if (dao.querySingle("title", title,Quest.class) ==null){
      Config.Builder config = Config.newBuilder().setAllowSharing(false)
          .setAutoAccept(false).setAutoClaim(false).setAutoConfirmAll(true)
          .setAutoConfirmFirstApplicant(true).setAutoReward(true)
          .setFavourite(false);
      Quest quest = new Quest(title,
          description, 1, config,
