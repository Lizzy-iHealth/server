...
public class APITest extends ModelTest {
  ...
  public void testApplyQuest() throws IOException {
    ...
    Applicant app = Applicant.newBuilder()
        .setType(Applicant.Status.WAIT_MY_CONFIRM).setUserId(friend.getId())
        .build();
  ...
  public void testShareQuest() throws IOException {
    ...
    assertEquals(friend.getId(), feedOfC.getFeeds().getFeed(0).getQuest()
        .getRefererId(0));
    assertEquals(user.getId(), feedOfC.getFeeds().getFeed(0).getQuest()
        .getOwnerId());
  ...
  public void testGetPhoneDetails() throws IOException {
    ...
    // TODO: should ask the user(if not friend) to prove information disclosure
    // for privacy
    ...
    when(req.getParameter(ParamKey.phone.name()))
        .thenReturn(newuser.getPhone());
  ...
  public void testInviteFriends() throws IOException {
    ...
    HttpServletRequest req = super.getMockRequestWithUser(user);
    HttpServletResponse resp = mock(HttpServletResponse.class);
    when(req.getParameterValues(ParamKey.phone.name()))
        .thenReturn(friend_phone);
    ...
    List<PendingUser> pu = dao.query(PendingUser.class).sortBy("phone", false)
        .prepare().asList();
  ...
  public void testAddFriends() throws IOException {
    ...
    for (int i = 0; i < users.length; i++) {
      users[i]
          .setDeviceID("APA91bFWFxgXtR57p3Jj2umYFFV8-U1N9PKKLQydheMybhU_2DxdngHbuYijPRHc1Y2a9dLkhdu9pyLCNd61uRBn9d2i6dggDxjMSkADyAET6rHGCQ9PFQi7HAc_hIsRBA_Z4LAkUddPSH9NxTvIjJZe-ImYHpoNgA");
      dao.save(users[i]);
    ...
    // add 3 friends, with a wrong user id in the middle: a add b, "5" and c,
